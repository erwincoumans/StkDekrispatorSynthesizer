...
{
	b3AlignedObjectArray<MySliderEventHandler<b3Scalar>*> m_sliderEventHandlers;
...
void GwenParameterInterface::registerSliderFloatParameter(SliderParams& params)
{
	...
	MySliderEventHandler<b3Scalar>* handler = new MySliderEventHandler<b3Scalar>(labelName,label,pSlider,params.m_paramValuePointer);
	m_paramInternalData->m_sliderEventHandlers.push_back(handler);

	pSlider->onValueChanged.Add( handler, &MySliderEventHandler<b3Scalar>::SliderMoved );
...
void GwenParameterInterface::syncParameters()
{
	...
	{
		MySliderEventHandler<b3Scalar>* handler = m_paramInternalData->m_sliderEventHandlers[i];
