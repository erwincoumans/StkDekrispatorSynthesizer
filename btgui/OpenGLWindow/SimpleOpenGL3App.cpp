...
void SimpleOpenGL3App::drawText3D( const char* txt, float posX, float posY, float posZ, float size)
{
...
void SimpleOpenGL3App::drawText( const char* txt, int posX, int posY)
{
	...
	} else
	{
		...
		while (txt[pos])
		{
			...
			//r.h = g_DefaultNormalFont->m_CharHeight;
			//r.w = g_DefaultNormalFont->m_CharWidth[c]+extraSpacing;
			int endX = startX+g_DefaultLargeFont->m_CharWidth[c];
			int endY = startY+g_DefaultLargeFont->m_CharHeight;
