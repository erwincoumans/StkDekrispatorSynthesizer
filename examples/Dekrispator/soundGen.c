...
{
	if ((noteG.automaticON1 || noteG.chRequested))
	{
		seq_sequence_new();
...
//	else
	...
	f0 = freq;//notesFreq[seq.track1.note[seq.step_idx]]; // Main "melody" frequency
	vol = frand_a_b(0.4f , .8f); // slightly random volume for each note
}
...
void make_sound_double(double *buf_double , uint16_t length) // To be used with the Sequencer
{
	...
	for (pos = 0; pos < length; pos++)
	{
		...
		*outp_double++ = yL;//valueL; // left channel sample
		*outp_double++ = yR;//valueR; // right channel sample
	}
