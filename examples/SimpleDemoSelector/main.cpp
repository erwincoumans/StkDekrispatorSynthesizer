...
struct TestRenderer : public CommonRenderInterface
{
	...
	GLfloat m_projMat[16];
	GLfloat m_viewMat[16];
	...
	virtual void updateCamera(int upAxis)
	{
		...
		{
			m_viewMat[i] = view[i];
			m_projMat[i] = projection[i];
		}
		glMatrixMode(GL_PROJECTION);
		glLoadIdentity();
		glMultMatrixf(m_projMat);

		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		glMultMatrixf(m_viewMat);
	}
	virtual void	getCameraViewMatrix(float viewMat[16]) const
	{
		for (int i=0;i<16;i++)
		{
			viewMat[i] = m_viewMat[i];
		}
	}
...
void selectDemo(int demoIndex)
{
	...
	int numDemos = sizeof(allDemoEntries)/sizeof(DemoEntry);
	...
	if (allDemoEntries[demoIndex].m_createFunc)
	{
		s_parameterInterface->removeAllParameters();
		sCurrentDemo = (*allDemoEntries[demoIndex].m_createFunc)(app,allDemoEntries[demoIndex].m_scenarioIndex);
		if (sCurrentDemo)
		{
			if (gui)
			{
...
int main(int argc, char* argv[])
{
	...
	//fill allDemos
	int numDemoEntries = sizeof(allDemoEntries)/sizeof(DemoEntry);
...
//	gui->getInternalData()->m_explorerPage
	...
	for (int d = 0; d<numDemoEntries; d++)
	{
//		sprintf(nodeText, "Node %d", i);
		Gwen::UnicodeString nodeUText = Gwen::Utility::StringToUnicode(allDemoEntries[d].m_name);
		if (allDemoEntries[d].m_menuLevel==1)
		{
